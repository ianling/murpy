...
class MessageType(metaclass=MurpyEnum):
    """
...
class Permission(metaclass=MurpyEnum):
    """
...
class PresetVoiceTarget(metaclass=MurpyEnum):
    DEFAULT = 0
    SERVER_LOOPBACK = 31


class AudioType(metaclass=MurpyEnum):
    """
...
class ConnectionState(metaclass=MurpyEnum):
    """
...
class EventType(metaclass=MurpyEnum):
    """
    The event types supported by Murpy.
    """
    ...
    """
    Fired when the Murpy instance's permissions in a channel have changed.
    """
    ...
    """
    Fired when the client receives the list of registered users on the server. These are stored in <Murpy instance>.registered_users
    """
